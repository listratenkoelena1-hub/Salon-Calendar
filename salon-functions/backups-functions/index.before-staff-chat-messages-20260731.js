...
exports.registerPushDevice = onCall(
  ...
  async (request) => {
    ...
    const previousTokens = await db.collection("staffPushDevices")
      .where("uid", "==", request.auth.uid)
      .where("enabled", "==", true)
      .get();
    const disableOldWrites = previousTokens.docs
      .filter(docSnap => docSnap.id !== deviceDocId)
      .filter(docSnap => {
        const device = docSnap.data() || {};
        return device.token === token || (safeDeviceId && device.deviceId === safeDeviceId);
      })
      .map(docSnap => docSnap.ref.set({
        enabled: false,
...
exports.staffNotificationCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    const data = snap.data() || {};
    if (data.pushEligible !== true || data.priority !== "key") {
      return;
    }
