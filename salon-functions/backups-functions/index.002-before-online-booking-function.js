...
function getAnyoneRemainingCapacity({
  ...
  const realStaff = staffRecords.filter(s => s.id && s.id !== ANYONE_ID);
...
function buildOnlineBookingTelegramMessage(data, staffRecords) {
  ...
  return `ONLINE BOOKING REQUEST\n${data.client} requested <b>${staffName}</b> - <u>${formatDate(data.date)}</u> at <i>${slotToTime(data.start)}</i>.${noteLine}${phoneLine}`;
}
...
exports.createOnlineBookingRequest = onCall(
  ...
  async (request) => {
    ...
    const selectedServices = Array.isArray(input.selectedServices)
      ...
      : [];
    const photoLinks = Array.isArray(input.photoLinks)
      ? input.photoLinks.map(item => String(item || "").trim()).filter(Boolean).slice(0, MAX_PHOTO_LINKS)
      : [];
