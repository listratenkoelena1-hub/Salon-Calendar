...
const callSalonLine = "If you need to change anything, please call the salon at 780-406-6767.";
...
if (eventType === "booking_confirmed") {
  ...
  return {
    ...
    text,
    html: `<p>Hi ${escapeHtml(client)},</p><p>Your appointment is confirmed${dateLine ? ` for ${escapeHtml(dateLine)}` : ""}.</p>${staffLine ? `<p>${escapeHtml(staffLine)}</p>` : ""}${serviceLine ? `<p>${escapeHtml(serviceLine)}</p>` : ""}<p>${escapeHtml(callSalonLine)}</p>`
  };
}
...
if (eventType === "booking_unavailable") {
  const text = plainLines([
    ...
    "If you don't hear from us soon, please call the salon at 780-406-6767."
  ]);
  return {
    subject: "Rose's Nails booking request update",
    text,
    html: `<p>Hi ${escapeHtml(client)},</p><p>The time you selected is no longer available or does not have enough time for the requested service.</p><p>We'll contact you by phone to find another option.</p><p>If you don't hear from us soon, please call the salon at 780-406-6767.</p>`
  };
}
...
return "This requested time is not available. Rose's Nails will contact you to find another time.";
}
...
function getAvailabilityForDate({
  ...
  const maxBookableStartSlot = Math.min(BOOKING_LATEST_START_SLOT, hours.end - BOOKING_DURATION);
  const realStaff = staffRecords
    .filter(s => s.id && s.id !== ANYONE_ID)
    .filter(s => s.bookingEnabled !== false)
    .map(s => ({
...
exports.appointmentStatusEmailUpdated = onDocumentUpdated(
  ...
  async (event) => {
    ...
    if (smsEventType && after.smsConsent !== false && after.phone) {
      const smsRef = db.collection(SMS_QUEUE_COLLECTION).doc(`${appointmentId}_${smsEventType}`);
