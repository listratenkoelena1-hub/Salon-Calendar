...
function getRequestedBookingDurationSlots(input = {}, staffRecord = {}) {
  ...
  segments.forEach(rawSegment => {
    ...
    if (isStandaloneRemoving) {
      if (textMatches(segment, [/\bacrylic\b/, /\bextension(s)?\b/, /\bnail(s)?\b/])) removingAddOn = Math.max(removingAddOn, 3);
    ...
    if (textMatches(segment, [/\bcut\s*nails?\b/, /\bnail\s*trim\b/])) otherAddOns += 1;
