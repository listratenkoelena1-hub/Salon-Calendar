...
Firebase Console хранит историю Rules и может восстановить предыдущий ruleset. Однако после появления приватных client-history документов прошлый main-ruleset восстанавливать нельзя: он содержит небезопасный общий fallback. Для нашего rollback нужно опубликовать подготовленный safe-файл как новый Rules release. Ни восстановление в Console, ни CLI-deploy не удаляют и не отменяют уже записанные данные.
...
## Что произойдёт при merge в main

Сам merge не публикует новые Rules или Functions. В текущем `main` GitHub Actions настроены так:

- push в отдельную ветку автоматически публикует только Hosting preview из `salon-calendar`;
- production Hosting запускается только вручную через `workflow_dispatch`;
- автоматического deploy Firestore Rules и Cloud Functions в workflow нет.

Поэтому ночной выпуск состоит из двух отдельных действий: сначала объединение проверенного кода с самым свежим `main`, затем явные Firebase deploy-команды в согласованном порядке. Один merge сам по себе Firebase backend не переключит.
